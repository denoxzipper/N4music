...
from N4.N4Utilities.database.sudo import get_sudoers, get_sudoers, remove_sudo, add_sudo
...
async def useradd(_, message: Message):
    if not message.reply_to_message:
        ...
        if added:
            ...
            return os.execvp("python3", ["python3", "-m", "N4"])
...
async def userdel(_, message: Message):
    if not message.reply_to_message:
        ...
        if user.id not in await get_sudoers():
            return await message.reply_text(f"❌ user is not a part of N4 Music")        
